...
if [ "y" = "$MULTISITE" ]
then
	ADMIN_PASSWORD=$(docker-compose exec --user www-data phpfpm wp core multisite-install --url=site1.local --title="$TITLE" --admin_user="$ADMIN_USER" --admin_email="$ADMIN_EMAIL")
else
	ADMIN_PASSWORD=$(docker-compose exec --user www-data phpfpm wp core install --url=site1.local --title="$TITLE" --admin_user="$ADMIN_USER"  --admin_email="$ADMIN_EMAIL")
fi
...
echo "$ADMIN_PASSWORD"
open https://site1.local/wp-login.php
